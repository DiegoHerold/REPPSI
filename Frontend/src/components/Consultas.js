...
function ConsultationsPage({ onPsychologistClick }) {
  ...
  const url = `http://localhost:5000/consultas/get?token=${token}`;
  ...
  const deleteConsultation = async (id) => {
    ...
    let url = `http://localhost:5000/consulta/apagar?idConsulta=${id}`
    try {
      const response = await fetch(url, {
        method: "DELETE",
        headers: {
  ...
  const deleteAllHistory = async (userId) => {
    ...
    let url = `http://localhost:5000/consultas/apagar?idUser=${userId}`
    try {
      const response = await fetch(url, {
        method: "DELETE",
        headers: {
  ...
  const cancelConsultation = async(id) => {
    ...
    let url = `http://localhost:5000/consulta/atualizar?idConsulta=${id}`
    try {
      const response = await fetch(url, {
        method: "POST",
        headers: {
  ...
  const entrarVideochamada = () => {
      console.log("entrando na videochamda com userId:") 
    ...
    <Box maxW="80%" mx="auto" p={6}>
            ...
            upcomingConsultations.map((consultation) => (
              ...
              >
                ...
                <Text mb={4} color="primary.300">
                  Psicólogo: {consultation.psychologistId}
                </Text>
                <Button as="a" href="#" size="sm" colorScheme="green" onClick={onOpen}>
                  Entrar na Chamada
                </Button>
                <IconButton
                  icon={<CloseIcon />}
          ...
          {consultationsHistory.length > 0 ? (
            consultationsHistory.map((consultation) => (
              ...
              >
                ...
                <Text color="primary.300">Psicólogo: {consultation.psychologistId}</Text>
                <Tag
                  size="lg"
                  mt={2}
                  bg={
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button colorScheme="red" mr={3} onClick={entrarVideochamada}>
              Confirmar
            </Button>
            <Button variant="ghost" onClick={onClose}>
              Cancelar
